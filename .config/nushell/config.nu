...
# ls aliases (using built-in ls with custom options)
alias l. = ls -a
alias la = ls -la
alias lD = ls -D
alias lf = ls -f
...
# Kubernetes aliases
alias ktl = kubectl
alias ktlg = kubectl get
alias ktll = kubectl logs
alias ktla = kubectl apply -f
alias ktld = kubectl delete
alias ktls = kubectl scale
alias ktlx = kubectl exec
...
# Rust aliases with pavex
alias cxr = cargo px run
alias cxrr = cargo px run --release
alias cxt = cargo px test
alias cxb = cargo px build
alias cxbr = cargo px build --release

# Rust aliases with lambda
alias clr = cargo lambda run
alias clrr = cargo lambda run --release
alias clt = cargo lambda test
alias clb = cargo lambda build
alias clbr = cargo lambda build --release

# OCaml aliases
alias op = opam
alias dn = dune
alias mg = mirage
alias dni = dune init project
alias ocx = opam exec -- dune exec
alias mgc = mirage configure -t unix
...
alias x = exec
alias j = just
alias jj = just --justfile ~/.config/just/justfile --working-directory .
alias ff = fastfetch
alias man = batman
alias cinema = asciinema
alias md = docling
...
def up [levels: int = 1] {
    cd ($"../" | repeat $levels | str join)
}

def pdf [input: path] {
    let output = ($input | path parse | update extension pdf | path join)
    ^pandoc $input -H ~/.config/pandoc/disable_float.tex -o $output
}

def fridge [method: string, endpoint: string, ...rest: string] {
...
# Generated by env.nu from the installed mise version.
use mise.nu
